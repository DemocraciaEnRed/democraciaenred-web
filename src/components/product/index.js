...
export default ({ data }) => {
    ...
    return (
        <section id={data.id} className={`hero section product product-${data.color}`}>
                            ...
                            <div className="column is-half">
                                <div className="is-flex title-product   is-justify-content-space-around is-align-items-center pb-4 mb-6">
                                    <figure className="image is-96x96 mr-3">
                                        <img src={`${icons[data.icon]}`} alt="" className={`${data.icon}`} />
                                    </figure>
                                    <h1 className="has-text-black is-spaced">{intl.formatMessage({id: data.title})}</h1>
                                </div>                               
                                <figure className="image is-hidden-desktop is-hidden-tablet">
                                    <img src={`${images[data.image]}`} alt="" />
                                </figure>
                                <p className="subtitle has-text-black  pt-4 is-spaced">{intl.formatMessage({id: data.description})}</p>
